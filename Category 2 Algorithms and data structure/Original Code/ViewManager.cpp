...
void ViewManager::ProcessKeyboardEvents()
{
	...
	// toggle projection on 'P' and 'O' keys
	if (glfwGetKey(m_pWindow, GLFW_KEY_P) == GLFW_PRESS) {
		ToggleProjection(); // Switch to perspective view
	}
	if (glfwGetKey(m_pWindow, GLFW_KEY_O) == GLFW_PRESS) {
		ToggleProjection(); // Switch to orthographic view
	}
}
/***********************************************************
* ProcessMouseScrollEvents()
*
* This methods is called to process any mouse scroll wheel
...
void ViewManager::ProcessMouseScroll(double yoffset)
{
	//Adjust the camera speed based on the scroll input
	if (yoffset > 0) {
		cameraSpeed += 0.01f; //increases the speed
	}
	else {
		cameraSpeed -= 0.01f; //decreases the speed
	}
...
void ViewManager::ToggleProjection() {
	...
	std::cout << "Current Projection: " << (currentProjection == PERSPECTIVE ? "Perspective" : "Orthographic") << std::endl;
}
...
void ViewManager::PrepareSceneView()
{
	...
	// Define the current projection matrix based on the selected projection type
	if (currentProjection == PERSPECTIVE) {
		projection = glm::perspective(glm::radians(g_pCamera->Zoom), (GLfloat)WINDOW_WIDTH / (GLfloat)WINDOW_HEIGHT, 0.1f, 100.0f);
	}
	else { // ORTHOGRAPHIC
